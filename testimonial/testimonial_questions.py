...
# --- Core Executor: The Testimonial Broadcaster ---
async def run_testimonial_cycle(bot: Bot, db, storage, question_id: int, test_mode: bool = True):
    ...
    if test_mode:
        ...
        prefix = ""
        # GATING LOGIC:
        # If Q_ID > 1, only select users who answered Q_ID - 1
        # Also ensure they haven't already answered the current Q_ID
        if question_id == 1:
            query = """
                SELECT u.telegram_id, u.language 
                FROM users u
                JOIN payments p ON u.telegram_id = p.user_id
                LEFT JOIN user_testimonials ut ON u.telegram_id = ut.user_id AND ut.question_id = $1
                WHERE p.status = 'approved' AND ut.user_id IS NULL
            ...
            query = """
                ...
                FROM users u
                JOIN payments p ON u.telegram_id = p.user_id
                INNER JOIN user_testimonials prev ON u.telegram_id = prev.user_id AND prev.question_id = $1 - 1
                LEFT JOIN user_testimonials current ON u.telegram_id = current.user_id AND current.question_id = $1
                WHERE p.status = 'approved' AND current.user_id IS NULL
                GROUP BY u.telegram_id, u.language
            """
            rows = await db._pool.fetch(query, question_id)

    sent_count = 0
    is_first = (question_id == 1) # Warm greeting only for the first question
    ...
    for user in rows:
        ...
        # Build the full personalized message
        ...
        try:
            ...
            logging.warning(f"Failed to send testimonial to {uid}: {e}")
...
async def testimonial_scheduler(bot: Bot, db, storage):
    ...
    while True:
        try:
            # 1. Check the last question actually BROADCASTED (even if not answered)
            last_sent = await db._pool.fetchrow("""
                SELECT question_id, sent_at FROM testimonial_logs 
                ORDER BY sent_at DESC LIMIT 1
            """)

            if last_sent:
                # Check if 6 hours have passed since the last send
                # (For testing, you can change 6 to 0.01 for ~36 seconds)
                elapsed = datetime.now(timezone.utc) - last_sent['sent_at']
                if elapsed.total_seconds() < (3 * 3600):
                    wait_time = (3 * 3600) - elapsed.total_seconds()
                    logging.info(f"⏳ Cooldown active. Waiting {int(wait_time)}s for next cycle.")
            ...
            next_q = await db._pool.fetchrow("""
                SELECT id FROM testimonial_questions 
                WHERE id > $1 AND is_active = TRUE 
                ORDER BY id ASC LIMIT 1
            """, last_q_id)

            if not next_q:
                # If we finished all questions, restart from 0 or stop
                logging.info("🏁 Survey Complete. Sleeping for 1 hour.")
                await asyncio.sleep(3600)
                continue

            q_id = next_q['id']
            
            # 3. Log the send BEFORE running the cycle to prevent race conditions
            ...
            logging.info(f"✅ Q{q_id} Broadcasted. Cycle locked for 6 hours.")
